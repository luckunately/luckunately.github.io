...
\begin{itemize}
      ...
      \item Processor utilization: It takes a long time for data to be fetched either from disk or outer source. Instead of stalling and waiting, it might be better to allow a different thread to operate. y having other threads, the program can do useful work while waiting for I/O. This use of multithreading is called masking (or hiding) I/O latency.
...
We can do so by:\begin{itemize}
      \item have operations to be done ``at-at-once'' or \textbf{atomically} so that we ensure operations are all independent and not interruptable. 
...
Note that StringBuffer is a thread\-safe data type where StringBuilder is not. Can be found in their specs. 

Collections.synchronizedMap(new HashMap<>()) is another example of synchronized wrapper. Now the operations are safe but iterators or for loops are still not safe. 
