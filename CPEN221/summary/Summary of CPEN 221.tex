...
In the old days, the compiler explicitly decides the address of data. However, the
challenge is that no other program can use it anymore.

The modern approach is to indicate an offset of the base address. It allows multiple
programs to run at the same time. Also, it allows the same data to be accessed by
different programs giving different offsets.

Compilers use offset from a stack pointer to track address. Stack is
\textbf{Last-in-first-out}. So last function called will be the first function
executed.

Data whose sizes cannot be known ahead of time will be stored in a heap. In JAVA,
objects are allocated on the heap. For example, ArrayLists and Sets that can
grow in size. Note: In JAVA, the array is actually a reference or pointer to the
array. So, the actual array is on the heap but the local variable array, which
is a pointer and holds a memory address, is stored on the stack.
...
housekeeping information that allows the called function (the callee) to return
to the caller safely.

\includegraphics*[scale = 0.3]{./summary_image/Stack_frame.png}

The stack pointer, \textbf{esp}, specifically points to the first byte occupied by
\textit{local buffer}, which is the loeser memory address in a stack containing
live data.

Base pointer or frame pointer, \textbf{ebp}, points to a fixed location within
the stack frame of the function currently running and provides a stable
reference point for accessing arguments and local variables. (Using offset)
Unlike esp, sbp is mostly maintained by program code with little CPU
interference.

Finally, the \textbf{eax} register is used by convention to transfer return
values back to the call for most C data types.
...
\begin{itemize}
      ...
      \includegraphics*{summary_image/Map java example.jpg}
...
For example, ``Comparator<T>'' object is a form of using an object to pass functions. 
