...
The Hamming ball in $\Sigma^n$ of radius $e$ about $x\in \Sigma^n$ is the set of all points in $\Sigma^n$ that are within $e$ distance from $x$. So $B_{\Sigma^n} (x,e)=\{y\in \Sigma^n: d(x,y)\le e\}$.


So the volume of the ball is $V=\sum_{i=0}^{\lfloor \frac{d-1}{2}\rfloor}\binom{n}{i}$. So the number of balls is $\frac{|\Sigma|^n}{V}$. So the number of codewords is $\frac{|\Sigma|^n}{V}$. So the rate is:
