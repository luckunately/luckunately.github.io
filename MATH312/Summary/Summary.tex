...
Each user chooses two large primes $p$ and $q$ and an encryption exponent $e$. The public key is $(n,e)$ where $n=pq$ and $e$ is the encryption exponent. The private key is $(n,d)$ where $d$ is the decryption exponent.
\begin{itemize}
    \item Encryption: $f(x)=x^e \pmod n$.
    \item Decryption: $g(x)=x^d \pmod n$. $d = e^{-1} \pmod {\phi(n)}$ where $\phi(n)=\phi(p)\phi(q)=(p-1)(q-1)$. 
\end{itemize}
\begin{theorem}
...
\end{cases}$. If we can do that then by CRT, we can show that $y\equiv x \pmod n$.\begin{proof}
    Apply Corollary 2 of Fermat's theorem. $ed\equiv 1 \pmod {(p-1)(q-1)}$. Then $ed=1 \begin{cases}
        \pmod {p-1} \\
        \pmod {q-1}
