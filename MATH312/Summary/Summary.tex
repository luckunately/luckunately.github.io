...
\end{cases}$
Let the prime decomposition of $n$ be $n=p_1^{a_1}\cdots p_r^{a_r}$, then $\mu(n)=0$ if $a_i\ge 2$ for some $i$ and $\mu(n)=(-1)^r$ if $n$ is a product of $r$ distinct primes where $a_i=1$ for all $i$.

Mobius Inversion Formula: \[
    f(n)=\sum_{d|n}F(d)\mu(\frac{n}{d})
\]
For example: $f(n)=1$ for every $n$, then $F(n)=\sum_{d|n}1=\tau(n)$. Then by the inversion formula: fefefefefef

Similarly, taking $f(n)=n$, we obtain efefefefefeffefe
