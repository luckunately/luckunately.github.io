...
In this case, the congruence has a solution if and only if $\gcd(a,n)=1$, which also means the congruence has exactly one solution.

Not all congruence classes have an inverse. For example, $ax\equiv 1 \pmod {10}$: \begin{align*}
...
Corollary: Let $p$ be a prime, then every $a\not\equiv 0 \pmod p$ has an inverse modulo $p$. It is because $\gcd(a,p)=1$.
...
\begin{proof}
    ...
    If $a\not \equiv 1$ then $a-1$ has a multiplicative inverse modulo $p$. Denote this multiplicative inverse by $b$. Time both side by $b$ Then $b(a-1)(a+1)\equiv 0 \pmod p \implies a+1\equiv 0 \pmod p \implies a\equiv -1 \pmod p$. Therefore, $a\equiv b \pmod p$.

    Similarly we can find that $a\not \equiv -1 \implies a\equiv 1 \pmod p$. So we proved the corollary.
\end{proof}
