...
\begin{proof}
    ...
    However, none of them is its own inverse except 1. It is because  $x^2\equiv 1 \pmod p \iff (x-1)(x+1)\equiv 0 \pmod p$. Then $x\equiv \pm 1 \pmod p$.

    Then we can pair up the elements in $S$ into pairs $(s,s^{-1})$. Then the product of each pair is congruent to 1 modulo $p$. However, the last term would not have a pair since there are odd numbers of terms in $S$ and 1 is its own pair.

    Therefore, $(p-1)!\equiv 1\times 2\times \cdots \times (p-2)\times (p-1) \equiv 1\times 2\times \cdots \times (p-2)\times 1 \equiv -1 \pmod p$.

    Only the term $\pm 1$ is its own inverse and left out. Therefore, the product of all the elements in $S$ is congruent to -1 modulo $p$.
\end{proof}
...
\begin{proof}
    ...
    \]
    Then by Wilson's Theorem, $(p-1)!\equiv -1 \pmod p$. Therefore, $a^{p-1}\equiv 1 \pmod p$.
\end{proof}
