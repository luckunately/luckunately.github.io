...
Compute $r_k\equiv 2^{k!} \pmod n$ recursively, using the formula $r_k\equiv r_{k-1}^{k} \pmod n$. For each $k$ compute $\gcd(r_{k-1},n)=g_k$. Note that $1\le g_k \le r_{k-1} \le n-2$ and $g_k$ divides $n$. So if $g_k > 1$, then we have found a factor of $n$ which is greater than 1.
...
In ohter words, p divides both $r_{k-1}$ and $n$ implies that $p$ divides $n$.
