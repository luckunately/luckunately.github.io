...
AVL Nodes are almost them same as a binary tree node. The only difference is that it contains an extra field to store the height info of the node. There are lots of ways to store the height info.
\begin{lstlisting}[language=C++]
...
\begin{itemize}
    ...
    \item A simple and ususally effictive has function is \begin{itemize}
