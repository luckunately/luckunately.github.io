...
DARM: random-access semiconductor memory. Usually consisting of a tiny \textbf{capacitor} and a \textbf{transistor}.
...
\begin{itemize}
    ...
    \item Stride Prefetching: Predict the stride (the constant difference between consecutive memory addresses) and prefetching data at regular intervals. Beneficial for non-contiguous but predictable memory access patterns.
          \begin{itemize}
              ...
              \item \includegraphics*{./Image/Stride FSM.png}
    ...
    \item Prefetching pros and cons:
          \begin{itemize}
              ...
              \item Imrove memory subsystem throughput, allowing the processor to continue executing instructions without waiting for memory accesses.
...
\begin{enumerate}
    ...
    \item Dynamic Insertion Policy BIP(DIP)
          ...
          \includegraphics*{./Image/DIP-SD.png}
...
\begin{itemize}
    ...
    \item Segmentation fault/ general protection fault
          \begin{itemize}
              ...
              \item Meltdown avoided this by misspeculating.
          \end{itemize}
\end{itemize}
...
\includegraphics*{./Image/VA cache miss.png}

\subsection*{Overlapping TLB and L1 cache accesses}

\includegraphics*{./Image/Overlapping TLB and L1 cache accesses.png}

Example problem:

Suppose pages are 4KB. D cache is 2\-way set-associative maximum cache size?
...
\includegraphics*{./Image/Simple pipeline.png}
...
\includegraphics*{./Image/Problematic pipeline.png}
...
\includegraphics*{./Image/pipeline control signal.png}
...
\includegraphics*{./Image/Pipeline Hazards.png}
...
Example:
\includegraphics*{./Image/Structural Hazard example.png}
...
\includegraphics*{./Image/Structural Hazard solution.png}
...
\begin{enumerate}
    \item Read after write (RAW) hazards:
          ...
          \includegraphics*{./Image/RAW instructions.png}
...
\begin{enumerate}
    ...
    \item Output dependency and WAW hazards:
          ...
          So it also results from the reuse of the name. If an output dependency causes a hazard in the pipeline, it is a WAW hazard.
\end{enumerate}
...
\includegraphics*[scale = 0.7]{./Image/ALU differnt length.png}
...
\begin{itemize}
    ...
    \item issue in\-order:
          ...
          \begin{itemize}
              \item if (RS.SB()$||$RT.SB()) RAW.stall();
              \item else if (RD.SB()) WAW.stall();
              \item else, dispatch to function Fn and set RD.SB to 1;
          \end{itemize}
\end{itemize}

For compelte out-of-order: Update GPR[RD], clear RD.SB();
...
\begin{itemize}
    ...
    \item Spirce register gets corresponding tags from RAT
...
\includegraphics*{Image/ROB figure.jpg}

\begin{itemize}
    \item sores instructions in \textbf{program order} (a FIFO)
...
So here, we have two CDBs that can handle two outputs at once. Then we run the load buffer at two times the speed or two assigning per cycle to allow two instructions adding per cycle. 
...
\includegraphics*{./Image/2 bit predictors.png}
...
\subsubsection*{1\-Bit Branch Predictor}
Now we can have one table for ``previously taken'' and one for ``previously not taken'':

\includegraphics*{./Image/one bit correlated branch example.jpg}
...
\includegraphics*{./Image/Shift Register.jpg}

We can keep the m most recent taken history inside this m shift registers which updates every time a branch outcome is computed. Then the output of the series of registers can be used to enter a mux that chooses which table out of the $2^m$ tables we need to use to predict. With the same logic, we can find which one to update when the outcome is out. 

These registers are call \textbf{(Global) Branch History Register} (G)BHR 

\section*{Tradeoffs in design}
...
\begin{itemize}
    \item queue stores ina \textbf{store buffer}
...
\includegraphics*{./Image/Store buffer.jpg}
...
Also, we need to note that we still do not have a way to check for \textbf{dependencies}. Before executing the load instructions, we need to check the \textbf{entire store buffer} to make sure that they are updated. 

\includegraphics*{./Image/Store Buffer Plus.jpg}

If the address matches a store buffer entry, we need to stall the load since we need to update the value. 

But when the value is actually in speculating, we should just get it anyway, which calls for forwarding.
...
\includegraphics*{./Image/load dependency predictor.png}
...
Multiple possible results could occur:\begin{align*}
    &x_1&x_2&\text{SC ordering}\\
    &1 &1 & A1,B1,A2,B2\\
    &1 &0 & B1,B2,A1,A2\\
    &0 &1 & A1,A2,B1,B2\\
    &0 &0 & \text{not possible} 
\end{align*}
...
\includegraphics*{./Image/Ilustrating WO.png}
...
\includegraphics*{./Image/Acquire and Release.png}

\begin{itemize}
    \item make sure no ld/st leak past Release
    \item make sure no ld/st leak before acquire 
\end{itemize}
...
\end{itemize}
from the bus. Note that all requests from the bus come from other cache controllers.
...
We do not have a copy $\implies$ send \textbf{read} request on the bus. \begin{itemize}
    \item If another cache has a block in the M state, it will downgrade to S and send us the latest value.
...
\begin{itemize}
    \item Block in I:\begin{itemize}
        ...
        \item If another cache has the block in M state, it will \textbf{invalidate} block and send us the latest value.
