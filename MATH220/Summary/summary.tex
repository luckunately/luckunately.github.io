...
\begin{proof}
    ...
    \begin{itemize}
        \item Assume a R v. We prove that $[a]\subseteq[b]$ and leave the other inclusion to the reader. Let $x\in [a]$, so that x R a. Since R is transitive and a R b, we know that x R b. Hence $x\in [b]$. The other inclusion is similar but also uses the symmetry of R.
...
\end{itemize}
Can refine $n>k$ case: at least one box has at least $\lceil n/k \rceil$ objects in it. 
...
Let $A=\{n\in\mathbb{N}s.t.n \text{ is even}\}$ and $B=\{n\in\mathbb{N}s.t.n \text{ is odd}\}$. Then $|A|=|B|=|\mathbb{N}|$ \begin{proof}
    ...
    The function $g: \mathbb{M} \to A$ defined by $g(n)=2n$ is a bijection.
\end{proof}
