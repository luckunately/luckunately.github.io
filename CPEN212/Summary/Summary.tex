...
\begin{itemize}
    ...
    \item For example, in C++, the destructor is called when the object goes out of scope, sockets and locks in Java and Python are closed when the object is garbage collected
...
We can use some logic to find the difference, for example, small numebrs are generally not pointers. However, this is not always true. If not pointing inside blocks, then it is not a heap pointer.
